-- 1. Enable the trigram extension for fuzzy text matching
...
BEGIN
  ...
  SELECT 
    ...
    (CASE 
      ...
      THEN (sqrt(power(cp.location_lat - p_user_lat, 2) + power(cp.location_lng - p_user_lng, 2)) * 111.32)
      ELSE NULL
    END)::DOUBLE PRECISION as distance_km,
    cp.rarity,
    (CASE 
      WHEN p_user_city IS NOT NULL AND p_user_country IS NOT NULL
      THEN similarity(concat(cp.location_city, ' ', cp.country), concat(p_user_city, ' ', p_user_country))
      ELSE 0.0
    END)::DOUBLE PRECISION as location_score
...
BEGIN
  ...
  ORDER BY
    (CASE 
      WHEN p_user_lat IS NOT NULL AND p_user_lng IS NOT NULL AND cp.location_lat IS NOT NULL AND cp.location_lng IS NOT NULL
      THEN (point(cp.location_lng, cp.location_lat) <-> point(p_user_lng, p_user_lat))
      ELSE 999999
    END),
    location_score DESC,
    cp.created_at DESC
